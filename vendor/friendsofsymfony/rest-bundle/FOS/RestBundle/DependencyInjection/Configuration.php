...
class Configuration implements ConfigurationInterface
{
    ...
    public function getConfigTreeBuilder()
    {
        ...
        $rootNode
            ...
            ->thenInvalid(
                'The param_fetcher_listener option does not support %s. Please choose one of '.json_encode(
                    $this->forceOptionValues
                )
            )
            ->end()
            ->end()
            ...
            ->ifTrue(
                function ($v) {
                    return !empty($v['version']) && !empty($v['groups']);
                }
            )
    ...
    private function addViewSection(ArrayNodeDefinition $rootNode)
    {
        $rootNode
            ...
            ->ifNotInArray($this->forceOptionValues)
            ->thenInvalid(
                'The view_response_listener option does not support %s. Please choose one of '.json_encode(
                    $this->forceOptionValues
                )
            )
            ->end()
    ...
    private function addExceptionSection(ArrayNodeDefinition $rootNode)
    {
        $rootNode
            ...
            ->thenInvalid(
                'Invalid HTTP code in fos_rest.exception.codes, see FOS\RestBundle\Util\Codes for all valid codes.'
            )
            ->end()
            ->prototype('scalar')->end()
            ->end()
