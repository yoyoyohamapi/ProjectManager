    ...
    public function testUsersFixture()
    {
        ...
        foreach ($etalonRoutes as $name => $params) {
            ...
            $this->assertEquals(
                ...
                'Method does not match for route: '.$name
            );
            $this->assertContains(
                $params['controller'],
                $route->getDefault('_controller'),
                'Controller does not match for route: '.$name
            );
        }
    }
    ...
    public function testAnnotatedUsersFixture()
    {
        ...
        foreach ($etalonRoutes as $name => $params) {
            ...
            $this->assertEquals(
                ...
                'requirements failed to match for '.$name
            );
            $this->assertContains(
                $params['controller'],
                $route->getDefault('_controller'),
                'controller failed to match for '.$name
            );
            if (isset($params['condition'])) {
                $this->assertEquals(
                    $params['condition'],
                    $route->getCondition(),
                    'condition failed to match for '.$name
                );
            }
