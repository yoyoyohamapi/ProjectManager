...
class ViewHandlerTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $viewHandler = new ViewHandler(
            array('html' => true, 'json' => false, 'xml' => false),
    ...
    public static function createResponseWithoutLocationDataProvider()
    {
        return array(
            ...
            'not templating aware and invalid form' => array(
                'json',
                array('data' => array(0 => 'error', 1 => 'error')),
                0,
                false,
                true
            ),
    ...
    public function testConfigurableViewHandlerInterface()
    {
        ...
        $this->assertEquals(
            array('bar'),
            $context->attributes->get('groups')->getOrThrow(new \Exception('Serialization groups not set as expected'))
        );
        $this->assertEquals(
            '1.1',
            $context->attributes->get('version')->getOrThrow(
                new \Exception('Serialization version not set as expected')
            )
        );
    ...
    public function testCreateResponseWithFormErrorsAndSerializationGroups($format)
    {
        ...
        $serializer = SerializerBuilder::create()
            ->configureHandlers(
                function (HandlerRegistry $handlerRegistry) use ($wrapperHandler, $formErrorHandler) {
