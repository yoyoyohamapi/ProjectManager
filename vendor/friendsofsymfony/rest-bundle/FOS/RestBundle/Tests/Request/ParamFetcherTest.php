...
class ParamFetcherTest extends \PHPUnit_Framework_TestCase
{
     ...
     */
    public function testValidatesConfiguredParam(
        $param,
        $expected,
        $expectedAll,
        $query,
                    ...
                    $validator->expects($self->at(0))
                        ...
                        ->with(
                            ...
                            new Regex(
                                array(
                                    'pattern' => '#^\\d+$#xsu',
                                    'message' => "Query parameter value 'bar', does not match requirements '\\d+'"
                                )
                            ),
                            null
                        )
                    ...
                    $validator->expects($self->at(1))
                        ...
                        ->with(
                            ...
                            new Regex(
                                array(
                                    'pattern' => '#^\\d+$#xsu',
                                    'message' => "Query parameter value 'invaliddata', does not match requirements '\\d+'"
                                )
                            ),
                            null
                        )
    ...
    public function testValidatesAddParam()
    {
        $queryFetcher = $this->getParamFetcher(
            array(),
            array('bar' => '2', 'baz' => '4', 'bub' => '10', 'arr' => array())
        );
        $queryFetcher->setController($this->controller);
    ...
    public function testExceptionOnValidatesFailure($query, $request, $param, \Closure $callback = null)
    {
        ...
        try {
            ...
            $this->fail(
                'Fetching in strict mode did not throw an Symfony\Component\HttpKernel\Exception\HttpException'
            );
        }
    }
                    ...
                    $validator->expects($self->at(0))
                        ...
                        ->with(
                            ...
                            new Regex(
                                array(
                                    'pattern' => '#^\\d+$#xsu',
                                    'message' => "Request parameter value 'foo', does not match requirements '\\d+'"
                                )
                            ),
                            null
                        )
    ...
    {
        $queryFetcher = new ParamFetcher(
            $this->paramReader, new Request(), $this->violationFormatter, $this->validator
        );
    ...
    public function testDeepArrayAllowedWithConstraint()
    {
        ...
        $request = new Request(
            array('bizoo' => array('foo' => array('b', 'a', 'r'))),
            array(),
            array('_controller' => __CLASS__.'::stubAction')
        );
        $reader = $this->getMockBuilder('FOS\RestBundle\Request\ParamReader')
            ->disableOriginalConstructor()
            ->getMock();
